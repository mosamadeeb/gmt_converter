    ...
    if dst_gmt.new_bones:
        ...
        for anm in in_file.animations:
            for bone in anm.bones:
                bone.name.update(NEW_BONES.get(bone.name.string(), bone.name.string()))
        if dst_gmt.is_dragon_engine:
            #rename to de
            for anm in in_file.animations:
                for bone in anm.bones:
                    bone.name.update(DE_FACE.get(bone.name.string(), bone.name.string()))
        """
        # add scale bone
        for anm in in_file.animations:
        ...
        for anm in in_file.animations:
            for bone in anm.bones:
                bone.name.update(OLD_BONES.get(bone.name.string(), bone.name.string()))
                bone.name.update(DE_FACE_OLD.get(bone.name.string(), bone.name.string()))
    
    if translation.reset:
        for anm in in_file.animations:
            anm.bones = reset_vector(anm.bones, src_gmt.new_bones, motion=motion)
...
def old_to_de_kosi(bones: List[Bone]) -> List[Bone]:
    ...
    for ke, ko in zip(ketu.rotation_curves(), kosi.rotation_curves()):
        ...
        for f in ko.graph.keyframes:
            ...
            ke_value = curve_array_to_quat(ke.curve_format, ke.values[ke.graph.keyframes.index(kf)])
...
def old_to_new_bones(bones: List[Bone], dst_de, motion, gmd_path) -> List[Bone]:
    ...
    if len(center):
        ...
        if dst_de:
            ...
            vector.curves = [c.to_horizontal() for c in deepcopy(center.position_curves())]
...
def reset_vector(bones: List[Bone], new_bones, is_de=True, motion=False, offset=None, add_offset=0.0):
    ...
    for name in names:
        ...
        v_pos.neutralize()
        v_pos.values = list(map(lambda v: (v[0] - offset[0], v[1] - offset[1] + height, v[2] - offset[2]), v_pos.values))
...
def translate_face_bones(anm_bones: List[Bone], source, target):
    ...
    for s, t in zip((face_s, jaw_s), (face_t, jaw_t)):
        for b_s in s.children:
            ...
            gmt_bone = [bone for bone in anm_bones if bone.name.string() == b_s.name]
            if not len(gmt_bone):
                continue
            gmt_bone = gmt_bone[0]
            ...
            s_pos = tuple(map(lambda x, y: x - y, s.global_pos, b_s.global_pos))
            t_pos = tuple(map(lambda x, y: -x + y, t.global_pos, b_t.global_pos))

            pos_curve.values = list(map(lambda f: [f[0] + s_pos[0] + t_pos[0], f[1] + s_pos[1] + t_pos[1], f[2] + s_pos[2] + t_pos[2]], pos_curve.values))
...
def transform_bones(anm_bones: List[Bone], new_bones, is_de, translation):
    ...
    if translation.reparent:
        ...
        for bone_t in target_gmd:
            ...
            if len(pos_curve):
                ...
                s_pos = tuple(map(lambda x, y: x - y, parent_s.global_pos, bone_s.global_pos))
                s_pos_new = tuple(map(lambda x, y: -x + y, parent_new.global_pos, bone_s.global_pos))
    ...
    def translate(start: str, stop=[]):
        ...
        if len(stop):
            for st in stop:
                ...
                if len(stop_s):
                    ...
                    stop_children.extend(list(map(lambda b: b.name, stop_s.children_recursive)))
                else:
                    stop_s = []
        ...
        for b_s in start_s.children_recursive:
            ...
            """
            s_pos = tuple(map(lambda x, y: x - y, p_s.global_pos, b_s.global_pos))
            t_pos = tuple(map(lambda x, y: -x + y, p_t.global_pos, b_t.global_pos))
            ...
            for side_name in ['_lip_side_r_n', '_lip_side_l_n']:
                ...
                side_pos.values = list(map(lambda f: \
                    [f[0] - btm_t.global_pos[0] + side_t.global_pos[0],
                     f[1] - btm_t.global_pos[1] + side_t.global_pos[1],
...
def combine(paths, ext):
    ...
    if ext == 'gmt':
        ...
        for path in paths[1:]:
            ...
            if result == -1:
                files.append((write_cmt_file(cmt_file, cmt_file.header.version), i))
                cmt_file = cmt_next
                i = 0
        if result != -1:
            files.append((write_cmt_file(cmt_file, cmt_file.header.version), i))
