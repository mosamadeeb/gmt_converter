...
def read_curves(gmt: BinaryReader, file: GMTFile) -> List[Curve]:
    ...
    for i in range(file.header.curve_count):
        ...
        curve.curve_format = parse_format(curve.property_fmt, curve.format, file.header.version)
        curve.values = read_animation_data(gmt, curve.curve_format, len(curve.graph.keyframes))
...
def read_animations(gmt: BinaryReader, file: GMTFile) -> List[Animation]:
    ...
    for i in range(file.header.anm_count):
        ...
        start = file.curves.index([b.curves for b in anm.bones if len(b.curves)][0][0])
